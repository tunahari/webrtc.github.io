...
const socket = io("http://localhost:3000"); // comment khi deploy gitpod
...
$("#btnCall").click(function () {
    ...
    socket.emit("CHECK_USER_EXIST",remotePeerId,(check)=>{
      ...
      openStream().then(stream => {
          ...
          } else if(check){ // Nếu tồn tại nhưng không ở cùng phòng
...
function toggleMic() {
    if (localStream) {
        ...
        document.getElementById("toggleMic").innerText = audioTracks[0].enabled
            ...
            : "Bật Mic";
        console.log(
            `Microphone ${audioTracks[0].enabled ? "enabled (Đã Bật)" : "disabled (Đã Tắt)"
            }`
        );
    }
}
...
function toggleCamera() {
    if (localStream) {
        ...
        document.getElementById("toggleCam").innerText = videoTracks[0].enabled
            ...
            : "Bật Camera";
        console.log(
            `Camera ${videoTracks[0].enabled ? "enabled (Đã Bật)" : "disabled (Đã Tắt)"
            }`
        );
    }
}
...
socket.on("SESSION_DESCRIPTION", ({ sdp, peerId }) => {
    if (peers[peerId]) {
        peers[peerId].peerConnection.setRemoteDescription(new RTCSessionDescription(sdp));
    }
});
